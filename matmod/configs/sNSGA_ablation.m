...
% algorithms =   {@sNSGAII         };
% mutation_op =  {@sparsePolyMutate};
% crossover =    {@cropover_v2     };
%                
% pop_samplers = {@nop};
% 
% labels = ["No striped pop sampling " ];
...
config = run_config(platPath,                                                 ...    %   platPath          
                    ...
                    100,                                                      ...    %   defaultDecVar     
                    0.1,                                                      ...    %   defaultSparsity   
                    [100, 200, 400, 800, 1600, 3200, 6400],                   ...    %   Dz         [100, 200, 400, 800, 1600, 3200, 6400],                 
                    linspace(0.05, 0.45,2),                                   ...    %   sparsities        (TODO revert)
                    "compDecVar",                                             ...    %   runType           
                    true,                                                     ...    %   saveData
                    'G:/My Drive/data/sNSGAIIRuns/',                          ...    %   savePath
                    false,                                                    ...    %   saveAllGens
                    @nop); 
